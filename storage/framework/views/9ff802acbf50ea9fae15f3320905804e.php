                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="border-b hover:bg-gray-50">
                                <td class="py-3 px-4"><?php echo e(\Carbon\Carbon::parse($transaction->tanggal_transaksi ?? $transaction->created_at)->format('d M Y')); ?></td>
                                <td class="py-3 px-4 text-gray-600"><?php echo e($transaction->keterangan ?: '-'); ?></td>
                                <td class="py-3 px-4 text-right font-bold text-green-600">Rp <?php echo e(number_format($transaction->total_penjualan ?? $transaction->total, 0, ',', '.')); ?></td>
                                <td class="py-3 px-4 text-center">
                                    <?php if($transaction->bukti): ?>
                                        <button 
                                            type="button"
                                            class="px-3 py-1 bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold rounded-md"
                                            data-img-url="<?php echo e(asset($transaction->bukti)); ?>">                                            Lihat
                                        </button>
                                    <?php else: ?>
